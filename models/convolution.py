...
class Conv2dStaticSamePadding(nn.Conv2d):
    def __init__(self, in_channels, out_channels, ksize, image_size=None, **kwargs):
        ...
        kh, kw = self.weight.size()[-2:]  #torch.Size([256, 64, 1, 1])
        sh, sw = self.stride
    ...
    def forward(self, x):
        x = self.static_padding(x)  #torch.Size([1, 64, 112, 112])
        x = F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)  ##self.weight torch.Size([256, 64, 1, 1]);
        return x  #x=torch.Size([1, 256, 112, 112])
...
class Conv2dBlock(nn.Module):
    ...
    def forward(self, x):#x=torch.Size([1, 64, 112, 112])
        x = self.conv(x)#x=torch.Size([1, 256, 112, 112])
        x = self.bn(x)
...
class DepthwiseConv2dBlock(nn.Module):
    ...
    def forward(self, x):#make_block depth=1 ;torch.Size([1, 384, 56, 56])
        x = self.conv(x)#torch.Size([1, 384, 56, 56])
        x = self.bn(x)
        if self.act_fn is not None:
            x = self.act_fn(x)
        return x
