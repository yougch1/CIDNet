...
def show_confMat(confusion_mat, classes, set_name, out_dir, epoch=999, verbose=False, figsize=None, perc=False):
    ...
    plt.yticks(xlocations, list(classes), rotation=60)
    ...
    # 保存
    plt.savefig(os.path.join(out_dir, "Confusion_Matrix_{}.svg".format(set_name)))
    plt.close()
...
def plot_line(train_x, train_y, valid_x, valid_y, mode, out_dir):
    ...
    plt.savefig(os.path.join(out_dir, mode + '.svg'))
    plt.close()


class Logger(object):
    def __init__(self, path_log):#'/Users/hello/PycharmProjects/MTX/results/12-21_11-09/log.log'
        log_name = os.path.basename(path_log)
        self.log_name = log_name if log_name else "root"#'log.log'
        self.out_path = path_log  #'/Users/hello/PycharmProjects/MTX/results/12-21_11-09/log.log'

        log_dir = os.path.dirname(self.out_path)#'/Users/hello/PycharmProjects/MTX/results/12-21_11-20'
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)

    def init_logger(self):
        logger = logging.getLogger(self.log_name)#<Logger log.log (WARNING)>
        logger.setLevel(level=logging.INFO)

        # 配置文件Handler <FileHandler /Users/hello/PycharmProjects/MTX/results/12-21_11-20/log.log
        file_handler = logging.FileHandler(self.out_path, 'w')#A handler class which writes formatted logging records to disk files.
        file_handler.setLevel(logging.INFO)
        formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s') #Formatter instances are used to convert a LogRecord to text.
        file_handler.setFormatter(formatter)#<FileHandler /Users/hello/PycharmProjects/MTX/results/12-21_11-20/log.log (INFO)>
...
def make_logger(out_dir):#'/Users/hello/PycharmProjects/MTX/results''/Users/hello/PycharmProjects/MTX/results'
    """
    ...
    time_str = datetime.strftime(now_time, '%m-%d_%H-%M')#'12-21_11-09'
    log_dir = os.path.join(out_dir, time_str)  # 根据config中的创建时间作为文件夹名：'/Users/hello/PycharmProjects/MTX/results/12-21_11-09'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    # 创建logger
    path_log = os.path.join(log_dir, "log.log")#'/Users/hello/PycharmProjects/MTX/results/12-21_11-20/log.log'
    logger = Logger(path_log)#<tools.common_tools.Logger object at 0x7fae2bf251c0>
